...
<script type="text/javascript">
	$(function (){
		$("#pageGo").click(function (){
			...
			window.location.href="<%=path%>/pyl/service!pageGo.action?index="+index+"&count="+count;
		...
		$("#serviceSearch").click(function (){
			...
			//alert(clientname);
			window.location.href="../pyl/service!search.action?clientname="+encodeURI(encodeURI(clientname))+"&summary="+encodeURI(encodeURI(summary))+"&condition.stid="+stid+"&condition.start="+start+"&condition.end="+end+"&condition.ssid="+ssid+"&id=3";
...
<table class="query_form_table">
	<tr>
		...
		<td>
			<select name="ssid">
				<option value="3" selected>已分配</option>
			</select>
		</td>
	</tr>
</table>
				...
				<c:forEach items="${requestScope.assignmentlist }" var="assignment">
					<c:if test="${assignment.servicetable.serid==list.serid }">
						<c:if test="${assignment.systemusertableBySuids.suid==sessionScope.users.suid }">
							<img onclick="to('../pyl/service!findAssignmentBySerid.action?serid=${list.serid }');" title="处理" src="<%=path %>/htmlCRM/html/images/bt_deal.gif" class="op_button" />  
						</c:if>
